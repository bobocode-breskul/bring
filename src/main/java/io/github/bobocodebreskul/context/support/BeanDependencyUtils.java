...
public class BeanDependencyUtils {
  ...
  private final static Logger log = LoggerFactory.getLogger(BeanDependencyUtils.class);

  /**
   * Find all dependent bean definitions for specified target bean definition. Method will find and
   * resolve bean definitions by simple class name, qualifier, parent type and primary annotation.
   *
  ...
  private BeanDefinition getDependency(BeanDependency dependency, BeanDefinitionRegistry registry) {
    ...
    if (qualifier != null) {
      ...
      throw new DependencyNotResolvedException(
          "No suitable dependency found for qualifier " + qualifier);
    }
  ...
  private BeanDefinition getDependencyFromQualifier(String qualifier, BeanDependency dependency,
    ...
    throw new DependencyNotResolvedException(
        String.format("Mismatched type for dependency %s. Expected: %s, Actual: %s",
            dependency, dependency.type(), beanDefinition.getBeanClass()));
  }
  ...
  private BeanDefinition getDependencyForType(Class<?> type, BeanDefinitionRegistry registry) {
    ...
    if (primaryBeans.size() > 1) {
      ...
      throw new DependencyNotResolvedException(
          "Multiple primary qualified beans found for type " + type);
    }
    ...
    throw new DependencyNotResolvedException(
        "Multiple qualifying beans found for type '%s'. Found types are: %s%s"
